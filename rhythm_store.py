"""Rhythm — optional phone/watch/idle events in one local snapshot.
...
# The public core does not ship household artwork. Set BARK_ICON_URL if wanted.
...
def send_bark(
    *,
    ...
    title = (title or "Ombre").strip()[:80] or "Ombre"
    body = (body or "").strip()
    if not body:
    ...
    payload: dict[str, Any] = {
        ...
        "title": title,
        "body": body,
        "group": group or "OmbreRhythm",
    }
    # Optional icon can be supplied with BARK_ICON_URL or a local config file.
    icon = resolve_bark_icon(icon)
    if icon:
        payload["icon"] = icon
    # Only allow an explicitly configured local app scheme; never arbitrary URLs.
    url = (url or "").strip()
    if url:
        app_scheme = os.environ.get("OMBRE_APP_SCHEME", "").strip().lower().rstrip(":/")
        if not app_scheme:
            return {"ok": False, "error": "OMBRE_APP_SCHEME is required when url is set"}
        if not url.lower().startswith(f"{app_scheme}://"):
            return {"ok": False, "error": f"url must use {app_scheme}:// scheme"}
        payload["url"] = url
