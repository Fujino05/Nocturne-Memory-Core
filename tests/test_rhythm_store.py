...
def test_public_core_has_no_bundled_bark_icon():
    assert DEFAULT_BARK_ICON_URL == ""
...
def test_send_bark_carries_configured_deep_link(monkeypatch):
    captured = {}
    monkeypatch.setenv("OMBRE_APP_SCHEME", "memoryapp")
    ...
    monkeypatch.setattr("rhythm_store.urllib.request.urlopen", fake_urlopen)
    result = send_bark(
        title="Nox",
        body="回来。",
        key="device-key",
    ...
    assert captured["payload"]["url"] == "memoryapp://chat"


def test_send_bark_rejects_external_deep_link(monkeypatch):
    monkeypatch.setenv("OMBRE_APP_SCHEME", "memoryapp")
    result = send_bark(
        title="Nox",
        body="回来。",
        key="device-key",
        url="https://example.com/not-home",
    )
    assert result == {"ok": False, "error": "url must use memoryapp:// scheme"}
